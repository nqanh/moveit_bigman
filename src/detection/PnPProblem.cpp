...
SUB (cv::Point3f v1,
     cv::Point3f v2);
cv::Point3f
get_nearest_3D_point (std::vector<cv::Point3f> &points_list,
                      cv::Point3f origin);
...
void
PnPProblem::set_P_matrix (const cv::Mat &R_matrix,
...
bool
PnPProblem::estimatePose (const std::vector<cv::Point3f> &list_points3d,
...
void
PnPProblem::estimatePoseRANSAC (const std::vector<cv::Point3f> &list_points3d,  // list with model 3D coordinates
                                const std::vector<cv::Point2f> &list_points2d,  // list with scene 2D coordinates
                                int flags,
                                cv::Mat &inliers,
                                int iterationsCount,  // PnP method; inliers container
                                float reprojectionError,
                                double confidence)    // Ransac parameters
{
  ...
  cv::solvePnPRansac (list_points3d, list_points2d, _A_matrix, distCoeffs, rvec, tvec, useExtrinsicGuess, iterationsCount, reprojectionError, confidence,
                      inliers, flags);
...
PnPProblem::verify_points (Mesh *mesh)
{
...
cv::Point2f
PnPProblem::backproject3DPoint (const cv::Point3f &point3d)
{
...
bool
PnPProblem::backproject2DPoint (const Mesh *mesh,
...
bool
PnPProblem::intersect_MollerTrumbore (Ray &Ray,
                                      Triangle &Triangle,
                                      double *out)
{
