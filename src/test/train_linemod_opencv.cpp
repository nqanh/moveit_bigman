...
readLinemod (const std::string& filename)
{
  ...
  //cv::FileStorage fs(filename, cv::FileStorage::READ); // NOT WORKING

  cv::FileStorage fs ("linemod_templates.yml", cv::FileStorage::READ);
  detector->read (fs.root ());

  cv::FileNode myClass = fs["classes"];

  cv::FileNodeIterator i = myClass.begin (), iend = myClass.end ();
  if (i == iend)
      ...
      char * argv[])
{
  ...
  if (argc == 1)
  {
    ...
    printf ("Creating linemod_templates.yml file\n");
  }
  else
  {
    printf ("Loading............................ \n");
...
static void
reprojectPoints (const std::vector<cv::Point3d>& proj,
             ...
             double f)
{
  ...
  for (int l_i = 0; l_i < (int) l_chain_vector.size (); ++l_i)
  {
    float l_dist = l_n[0] * CV_MAT_ELEM(*lp_pts, float, l_i, 0) + l_n[1] * CV_MAT_ELEM(*lp_pts, float, l_i, 1) + l_n[2] * CV_MAT_ELEM(*lp_pts, float, l_i, 2)
