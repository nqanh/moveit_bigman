      ...
      char** argv)
{
  ...
  parse_argument (argc, argv, "-g", grad_mag_thresh);

  // Parse the detection threshold
  float detect_thresh = 0.75f;
  parse_argument (argc, argv, "-d", detect_thresh);
  ...
  if (parse_argument (argc, argv, "-i", input_filename) < 0)
    return (-1);
  ...
  for (size_t i = 0; i < detections.size (); ++i)
  {
    ...
    print_info ("%lu %lu %f (%f %f %f) (%f %f %f)\n", d.detection_id, d.template_id, d.response, bb.x, bb.y, bb.z, bb.width, bb.height, bb.depth);
  }

  print_info ("Can't detect object\n");
}
